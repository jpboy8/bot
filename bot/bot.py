...
change_surname_btn = InlineKeyboardButton(
    text="Изменить фамилию", callback_data="change_surname"
)
change_phone_number_btn = InlineKeyboardButton(
    text="Изменить номер телефона", callback_data="change_phone_number"
)
...
async def handle_change_phone_number_message(message: types.Message, state=FSMContext):
    ...
    if re.match(pattern, phone_number) is not None:
        ...
        bot.send_message(
            message.from_user.id, "Номера телефона не соответствует формату"
        )
...
async def change_phone_nub_btn(callback: types.CallbackQuery):
    if not db.user_exists(callback.from_user.id):
        ...
        await bot.send_message(
            ...
            "Введите ваш номер телефона с кодом страны и без пробелов. Пример для заполнения: +77017299556 ",
        )
...
async def change_surname_btn(callback: types.CallbackQuery):
    if not db.user_exists(callback.from_user.id):
        ...
        await bot.send_message(
            callback.from_user.id, "Введите фамилию на русском языке: "
        )
...
async def show_profile(callback: types.CallbackQuery):
    if not db.user_exists(callback.from_user.id):
        ...
        await bot.send_message(
            callback.from_user.id, db.show_profile(callback.from_user.id)
        )
        await callback.answer()
...
async def handle_name_message(message: types.Message, state=FSMContext):
    ...
    await state.finish()
    await bot.send_message(
        message.from_user.id, "Введите вашу фамилию на русском языке: "
    )
...
async def handle_reg_command(callback: types.CallbackQuery):
    if not db.user_exists(callback.from_user.id):
        ...
        await bot.send_message(
            callback.from_user.id, "Введите ваше имя на русском языке: "
        )
    ...
    if message.chat.id == adm_chat_id:
        ...
        if member["status"] == "creator" or member["status"] == "administrator":
            if not db.chat_exists(message.chat.id):
                ...
                await bot.send_message(
                    message.chat.id, "Группа успешно добавлена в базу"
                )
            else:
        ...
        if member["status"] == "creator" or member["status"] == "administrator":
            try:
                ...
                for group in groups:
                    ...
                    await bot.send_message(
                        message.chat.id,
                        f"Пользователь был успешно удален из группы {group[0]}",
                    )
